...
//        return subscripcions;
    ...
    public void addProyecto(Proyecto proyecto) throws SQLException {
        ...
        statement.setInt(2, proyecto.getId());
    ...
    public Proyecto getProyecto(String nombreProyecto) throws SQLException {
        PreparedStatement getMetodos = connection.prepareStatement("SELECT * FROM metodos WHERE proyecto = ?");
        getMetodos.setString(1, nombreProyecto);

        ResultSet resultMetodos = getMetodos.executeQuery();
        ArrayList<Metodo> metodos = new ArrayList<>();

        while (resultMetodos.next()) {
            metodos.add(new Metodo(resultMetodos.getString("nombre"), resultMetodos.getInt("eficacia"), resultMetodos.getInt("tiempo")));
        }

        PreparedStatement getProyecto = connection.prepareStatement("SELECT * FROM proyectos WHERE nombre=?");
        getProyecto.setString(1, nombreProyecto);

        ResultSet resultProyecto = getProyecto.executeQuery();
        Proyecto proyecto;
        if (resultProyecto.first())
            proyecto = new Proyecto(
                    resultProyecto.getInt("id"),
                    resultProyecto.getString("nombre"),
                    resultProyecto.getString("codigo"),
                    resultProyecto.getInt("usuario"),
                    metodos
            );
        else
            throw new SQLException("Error desconocido: consulte con soporte");

        getProyecto.close();
        resultProyecto.close();
        resultMetodos.close();
        getMetodos.close();
        return proyecto;
    }
